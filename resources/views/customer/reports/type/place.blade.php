                                ...
                                <div class="row col-12 p-0 m-0 mb-3 menu-and-filter menu-and-filter--custom">
                                    <div class="col-8">
                                        <ul class="nav nav-tabs nav-tabs--custom" id="myTab" role="tablist">
                                            ...
                                            <li class="nav-item">
                                                <a class="nav-link active" href="@if(request('show_by') != null) {{route('reports.show',array_merge(['type'=>'place'], request()->toArray()))}} @else {{ route('reports.show','place')}} @endif">{{ __('app.Bay_Area') }}</a>
                                            </li>
                                            <li class="nav-item">
                                    ...
                                    <div class="col-4">
                                        <div class="d-flex justify-content-end position-relative filter-cont">
                                            @include('customer.reports.extra._filter',['type' => 'place'])
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        @if(in_array($key_name ,array_values($config['chart']['bar'])))
            @if(count($report['charts']))
                ...
                barChart('BranchPlaceBar',charts.bar, info);
            @endif
        @endif
       /**************** End Bar Chart****************/
       ...
       /**************** Start Line Chart ************/
        @if(in_array($key_name ,array_values($config['chart']['line'])))
            @if(count($report['charts']))
                ...
                lineChart('BranchPlaceLine',charts.bar, info);
            @endif
        @endif
        /************** End Line Chart ************/
        ...
        @if(in_array($key_name ,array_values($config['chart']['side_bar'])))
            @if(count($report['charts']))
                ...
                sideBarChart('BranchPlaceSideBar',charts.bar, info);
            @endif
        @endif
        /************** End SideBar Chart ************/
        ...
        @if(in_array($key_name ,array_values($config['chart']['circle'])))
            @if(count($report['charts']))
                ...
                info.columns.forEach(function (col) {
                    pieChart(`PlaceCircle${col}`,charts.circle[col],info)
                });
             @endif
        @endif
        /**************** End Circle Chart ***************/
        ...
        @if(in_array($key_name ,array_values($config['chart']['trend_line']??[])))
            @if(count($report['charts']))
                ...
                trendLineChart('BranchPlaceTrendLine',charts.bar, info);
            @endif
        @endif
        /**************** End TrendLine Chart****************/

        /********************* Start Dynamic Chart *********/
{{--        @if($show_by == 'comparison')--}}
{{--            @if(in_array($key_name ,array_values($config['chart']['circle'])))--}}
{{--                @if(count($report['charts']) && false)--}}
{{--                    @if(diffMonth($report['charts']['dynamic_bar']['start_at'],$report['charts']['dynamic_bar']['end_at']) > 1)--}}
{{--                        $("#BranchPlaceDynamicBarCon").show();--}}
{{--                        dynamicBar('BranchPlaceDynamicBar', @json($report['charts']['dynamic_bar']['data']),"{{$report['charts']['dynamic_bar']['start_at']}}","{{$report['charts']['dynamic_bar']['end_at']}}");--}}
{{--                    @endif--}}
{{--                @endif--}}
{{--            @endif--}}
{{--        @endif--}}
