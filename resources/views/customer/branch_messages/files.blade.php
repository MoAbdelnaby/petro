                                    ...
                                    <div class="related-heading mb-5">
                                        ...
                                        <span>
                                            <form action="{{ route('user_settings',['show_items']) }}" method="post">
                                                ...
                                                <button type="submit">
                                                    <i class="fas fa-table {{ $userSettings ? ($userSettings->show_items == "table" ? 'active' : '') :''}}"></i></button>
                                            </form>
                                            <form action="{{ route('user_settings',['show_items']) }}" method="post">
                                                @csrf
                                                ...
                                                <table
                                                    ...
                                                    @foreach($items as $item)
                                                        <tr class="item{{$item->id}}">
                                                            ...
                                                            <td>{{$item->start??'First Date'}}</td>
                                                            <td>{{$item->end??'Last Date'}}</td>
                                                            <td>{{$item->branch->name ?? ''}}</td>
                                                            <td>
                                                                @if($item->status)
                                                                    <span class="badge badge-success font-size-12">Ready</span>
                                                                @else
                                                                    <span class="badge badge-danger font-size-12">Not Ready</span>
                                                                @endif
                                                            </td>
                                                            <td>{{$item->type }}</td>
                                                            <td>
                                                                @if(in_array(auth()->user()->type,['subadmin','customer','subcustomer']))
                                                                    ...
                                                                    @endif
                                                                    <a class="btn btn-sm btn-outline-danger" style="color: red"
                                                                        ...
                                                                        @if($item->status && Storage::disk('public')->exists($item->url))
                                                                            <a class="btn btn-sm btn-primary"
                                                                               href="{{Storage::disk('public')->url($item->url) }}">
                                                                                {{__('app.customers.branches.download')}}
                                                                            </a>
                                                                        @endif
                                                                        <a class="btn btn-outline-danger" style="color: red" data-toggle="tooltip"
                                                                           data-placement="top" title="Delete"
                                                                           data-original-title="Delete"
                                                                           onclick="delete_alert({{ $item->id }});">
                                                                            {{__('app.customers.branches.delete')}}
                                                                        </a>
                                                                    @endif
                                                                </div>
                                                            </div>
