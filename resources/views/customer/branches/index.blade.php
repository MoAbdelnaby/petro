                            ...
                            <div class="row">
                                <div class="col-sm-12 mt-3 related-product">
                                    <div class="related-heading mb-5">
                                        <h2>
                                            <img src="{{resolveDark()}}/img/branch.svg" width="30" alt="product-image" class="img-fluid"></a>
                                            {{__('app.customers.branches.page_title.index')}}
                                        </h2>
                                        <span>
                                                ...
                                                <table class="table dataTable ui celled table-bordered text-center no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                                    ...
                                                    @foreach($items as $item)
                                                        <tr class="item{{$item->id}}">
                                                            <td>
                                                                <img src="{{ $item->photo ? url('storage/'.$item->photo): ( session()->has('darkMode') ? url('/images/models/dark/branch.svg'):url('/images/models/default/branch.svg') )}}"  width="auto" height="30px"  alt="product-image" class="img-fluid">
                                                            </td>
                                                            <td>{{$item->name}}</td>
                                                            <td>{{$item->code}}</td>
                                                            ...
                                                            <td>
                                                                @if(auth()->user()->type=="customer")
                                                                    <a class="btn btn-sm btn-info" href="{{ route('customerBranches.show',[$item->id]) }}">{{__('app.customers.branches.show')}}</a>
                                                                    <a class="btn btn-sm btn-primary" href="{{ route('customerBranches.edit',[$item->id]) }}">{{__('app.customers.branches.edit')}}</a>
                                                                    <a class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="delete_alert({{ $item->id }});" >{{__('app.customers.branches.delete')}}</a>
                                                                @endif
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                        ...
                                                        <div class="iq-card">
                                                            <div class="product-miniature">
                                                                <div class="thumbnail-container text-center pb-0">
                                                                    <img src="{{ $item->photo ? url('storage/'.$item->photo): ( session()->has('darkMode') ? url('/images/models/dark/branch.svg'):url('/images/models/default/branch.svg') )}}"  width="auto" height="100"  alt="product-image" class="img-fluid">

                                                                </div>
                                                                <div class="product-description text-center">
                                                                    <h5><small>{{$item->name}}</small></h5>
                                                                    <h5><small><span><i class="fas fa-key"></i> {{__('app.customers.branches.table.code')}} </span> : {{$item->code}}</small></h5>
                                                                    <h5><small><span><i class="fas fa-oil-can"></i> {{__('app.customers.branches.table.area_count')}}</span> : {{$item->area_count}}</small></h5>
                                                                    <h5><small><span><i class="fas fa-map-marker-alt"></i> {{__('app.region')}}</span> : {{$item->region->name ?? ''}}</small></h5>
                                                                    <h5 class="d-block">
                                                                        <a href="{{ route('branches.change_active',[$item->id]) }}">
                                                                            @if($item->active==1)
                                                                                <i class="fas fa-check-square {{$item->active==1 ? 'True':'False'}}"></i>
                                                            ...
                                                            <div class="ratting-item-div">
                                                                ...
                                                                <div class="ratting-item d-flex align-items-center justify-content-center p-0 m-0 pb-2">
                                                                    @if(auth()->user()->type=="customer")
                                                                        <a class="btn btn-info" href="{{ route('customerBranches.show',[$item->id]) }}">{{__('app.customers.branches.show')}}</a>
                                                                        <a class="btn btn-primary mx-1" href="{{ route('customerBranches.edit',[$item->id]) }}">{{__('app.customers.branches.edit')}}</a>
                                                                        <a class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="delete_alert({{ $item->id }});" >{{__('app.customers.branches.delete')}}</a>
                                                                    @endif
                                                                </div>
                ...
                <div class="modal-dialog modal-md modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger" onclick="delete_option('customer/customerBranches');">{{__('app.customers.branches.delete')}}</button>
                        </div>
                    </div>
                </div>
            </div>
