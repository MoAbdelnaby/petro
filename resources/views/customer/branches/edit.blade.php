                    ...
                    <div class="iq-card">
                        ...
                        <div class="iq-card-body">
                            <div class="new-user-info">
                                <form  id="branchForm" method="POST" action="{{ route('customerBranches.update',[$id]) }}" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 row">
                                            ...
                                            <div class="form-group col-sm-12 col-md-6 col-lg-6">
                                                ...
                                                <input  type="number" name="code" class="form-control" id="code" placeholder="{{__('app.customers.branches.table.code')}}" value="{{ old('code')? old('code') : $item->code }}">
                                                @error('code')
                                                <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            ...
                                            <div class="form-group col-sm-12 col-md-6 col-lg-6">
                                                ...
                                                <input  type="number" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"
                                                       maxlength = "1" name="area_count" class="form-control" id="area_count" placeholder="{{__('app.customers.branches.table.area_count')}}" value="{{ old('area_count')? old('area_count') : $item->area_count }}">
                                                @error('area_count')
                                                <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            ...
                                            <div class="col-md-6 row p-0 m-0">
                                                <div class="form-group col-sm-12 col-md-6 col-lg-6">
                                                    ...
                                                    <div class="input-group">
                                                        <input type="number" name="top" class="top form-control" disabled="disabled" id="top" step=".1" min="0" max="100" value="{{ $item->top }}">
                                                ...
                                                <div class="form-group col-sm-12 col-md-6 col-lg-6">
                                                    ...
                                                    <div class="input-group">
                                                        <input type="number" name="left" class="top form-control" disabled="disabled" id="left" step=".1" min="0" max="100" value="{{ $item->left }}">
                                                        <div class="input-group-append">
                                                            <span class="input-group-text">%</span>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="form-group col-sm-12 col-md-6 col-lg-6">
                                                ...
                                                <select class="form-control branch-reg-edit" required name='region_id' >
                                                    @foreach ($regions as $reg)
                                                        <option data-reg="{{$reg->branches}}" data-img="{{ $reg->photo ?? '' }}" value="{{ $reg->id }}" {{ $item->region_id == $reg->id ? 'selected' : '' }}>{{ $reg->name}}</option>
                                                    @endforeach
                                                </select>
                                                @error('region')
                                                <span class="invalid-feedback" role="alert">
                                        ...
                                        <div class=" reg-img form-group col-sm-12 col-md-6">
                                            <img src="{{ $item->region ? url("storage/".$item->region->photo ) : asset('/images/select-region.svg') }}" class="regionImage" style="display: block" >
                                            <img src="{{ asset('images/mark_blue.png')}}" width="20" class="draggable" style="left: {{$item->left}}%; top: {{$item->top}}%;" alt="">
                                            @if($item->region)
                                                @foreach($item->region->branches as $branch)
                                                    @if($branch->name != $item->name)
                                                        <img src="{{ asset('images/mark_black.png')}}" width="20" class="mark_black" title="{{ $branch->name }}" style="position: absolute;left: {{$branch->left}}%; top: {{$branch->top}}%;" alt="">
                                                    @endif
                                                @endforeach
                                            @endif
