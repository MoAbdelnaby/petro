...
@push('css')
<!-- required css -->
<link href="{{ asset('/themed/hud/assets/plugins/bootstrap-table/dist/bootstrap-table.min.css') }}" rel="stylesheet" />
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/map_main.css') }}">
@endpush
@section('content')
...
<div id="content" class="app-content">
    ...
    <li class="breadcrumb-item active">{{ __('app.Map') }}</li>
    </ul>
    <h1 class="page-header">
        <i class="fas fa-map-marker-alt"></i> {{ __('app.Map') }}
    </h1>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <ul class="nav nav-tabs">
                <li class="nav-item me-1"><a href="#Region" class="nav-link active" data-bs-toggle="tab">{{ __('app.Regions') }}</a></li>
                <li class="nav-item me-1"><a href="#Branch" class="nav-link" data-bs-toggle="tab">{{ __('app.Branches') }}</a></li>
                <li class="nav-item me-1"><a href="#Plates" class="nav-link" data-bs-toggle="tab">{{ __('app.Plates') }}</a></li>
            </ul>
            <div class="tab-content pt-3">
                <div class="tab-pane fade show active" id="Region">
                    <form id="mapfilter" action="{{route('map.filter')}}" method="post" class="row col-12 m-0 p-0">
                        @csrf
                        <input type="radio" style="display: none" id="Region" value="regions" name="table" checked>
                        <input type="search" autocomplete="off" name="searchItem" class="search form-control" placeholder="{{ __('app.Search') }}">
                    </form>
                </div>
                <div class="tab-pane fade" id="Branch">
                    <form id="mapfilter" action="{{route('map.filter')}}" method="post" class="row col-12 m-0 p-0">
                        @csrf
                        <input type="radio" style="display: none" id="Branch" value="branches" name="table" checked>
                        <input type="search" autocomplete="off" name="searchItem" class="search form-control" placeholder="{{ __('app.Search') }}">
                    </form>
                </div>
                <div class="tab-pane fade" id="Plates">
                    <form id="mapfilter" action="{{route('map.filter')}}" method="post" class="row col-12 m-0 p-0">
                        @csrf
                        <input  type="radio" style="display: none" id="plate" value="carprofiles" name="table" checked>
                        <input type="search" autocomplete="off" name="searchItem" class="search form-control" placeholder="{{ __('app.Search') }}">
                    </form>
            ...
            <div class="row">
                <div class="col-3">
                    <ul class="nav nav-tabs list-group">
                        <li class="">
                            <a class="nav-item list-group-item list-group-item-action active" id="firstMap-tab" data-bs-toggle="tab" href="#firstMap" role="tab">
                                {{ __('app.Saudi_Map') }}
                            </a>
                        </li>
                        @foreach($regions as $key=>$region)
                            @php
                                $name = strtolower($region->display_name);
                                $link_id = str_replace(' ','_',$name);
                            @endphp
                            <li class="">
                                <a class="nav-item list-group-item list-group-item-action " id="{{ $link_id }}-tab" data-bs-toggle="tab" href="#{{ $link_id }}">
                                    {{ $region->name }}
                                    <span class="float-right">
                                        {{ $region->branches_count }}
                                    </span>
                                </a>
                            </li>
                        @endforeach
                ...
                <div class="col-9">
                    <div class="tab-content pt-3">
                        <div class="tab-pane fade active show" id="firstMap">
                            <div class=" reg-img form-group" style="text-align: center;height: 530px;width: 100%;position: relative">
                                <svg

                                    width="730.13947"
                                    height="600.4032">

                                    <path class="riyadh_region" fill="#96ebff"
                                          ...
                                          title="Ar Riyāḑ"
                                          id="SA-01" />
                                    <path class="makkah_region" fill="#0769b3"
                                          ...
                                          title="Makkah"
                                          id="SA-02" />
                                    <path class="al_madinah_region" fill="#1ab7fe"
                                          ...
                                          title="Al Madīnah"
                                          id="SA-03" />
                                    <path class="al_sharqiyah_region" fill="#0583e9"
                                          ...
                                          title="Ash Sharqīyah"
                                          id="SA-04" />
                                    <path class="alqussim_region" fill="#0483e6"
                                          ...
                                          title="Al Qaşīm"
                                          id="SA-05" />
                                    <path class="hail_region" fill="#0568b9"
                                          ...
                                          title="ha'il_region"
                                          id="SA-06" />
                                    <path class="tabuk_region" fill="#96ebff"
                                          ...
                                          title="Tabūk"
                                          id="SA-07" />
                                    <path class="al_hudud_al_shamaliyah" fill="#1ab7fe"
                                          ...
                                          id="SA-08" />
                                    <path class="jizan_region" fill="#98eaff"
                                    ...
                                    <path class="najran" fill="#0568b9"
                                          ...
                                          title="Najrān"
                                          id="SA-10" />
                                    <path class="bahah_region" fill="#0a8ade"
                                          ...
                                          title="Al Bāḩah"
                                          id="SA-11" />
                                    <path class="aljawf_region" fill="#0583e9"
                                          ...
                                          title="Al Jawf"
                                          id="SA-12" />
                                    <path class="asir_region" fill="#18b8fe"
                                ...
                                {{--                                                <svg src="/images/saudi-arabia.svg" class="regionImage"></svg>--}}
                                {{--                                                <img src="/images/saudi-arabia.svg" class="regionImage" style="display: block" >--}}
                            </div>
                        </div>
                                ...
                                <div class=" reg-img form-group" style="height: 530px;width: 700px;margin:0 auto;position: relative">
                                    ...
                                    @foreach($region->branches as $branch)
                                        <div class="markMap"  style="position: absolute;left:{{$branch->left}}% ; top: {{$branch->top}}%;">
                                            ...
                                            <span class="linkMark"><a href="{{ route('customerBranches.show',[$branch->id]) }}" target="_blank"> <i class="fa fa-link"></i> {{ $branch->name }}</a></span>
                                        </div>
                                    @endforeach
                    ...
                    <div class="clearfix col-12 clear-both my-5 pt-5"></div>
                    <div class="carPlatesDataTable" style="display:none">
                        <div class="showImageCarPlates" >
                        ...
                        <table id="datatableDefault" class="table text-nowrap w-100">
                            ...
                            <thead>
                                ...
                                <th>{{ __('app.Plate_Color') }}</th>
                                <th>{{ __('app.Plate_Country') }}</th>
                                <th>{{ __('app.Plate_Ar') }}</th>
                                <th>{{ __('app.Plate_En') }}</th>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>{{ __('app.Image') }}</th>
...
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net/js/jquery.dataTables.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-bs5/js/dataTables.bootstrap5.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons/js/dataTables.buttons.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons/js/buttons.colVis.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons/js/buttons.flash.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons/js/buttons.html5.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons/js/buttons.print.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-buttons-bs5/js/buttons.bootstrap5.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-responsive/js/dataTables.responsive.min.js') }}"></script>
<script src="{{ asset('/themed/hud/assets/plugins/datatables.net-responsive-bs5/js/responsive.bootstrap5.min.js') }}"></script>
...
<script>
    ...
    <script>
        $(document).ready(function(){
            var theme = "{{ session('darkMode') }}";
            var urlBase="";
            var noresualt = "{{ __('app.No_Result_Found') }}";

            console.log(noresualt)
            if(theme == ""){
                urlBase = "/images/";
            }
            else if(theme == "on"){
            ...
            function getSearchData() {
                ...
                if(ll.length > 2){
                        ...
                        success: function (data) {
                            ...
                            if(data.data.length > 0) {
                                ...
                                for (var r = 0; r < data.data.length; r++) {
                                    ...
                                    var classes;
                                    if (table === "regions") {
                                        regname = data.data[r].name.toLowerCase();
                                        regid = regname.replaceAll(' ', '_');
                                        classes = "normal";
                                    }
                                    if (table === "branches") {
                                        if (data.data[r].region && data.data[r].region.name) {
                                            regname = data.data[r].region.name.toLowerCase();
                                            regid = regname.replaceAll(' ', '_');
                                            classes = "normal";
                                        }
                                    }
                                    if (table === "carprofiles") {
                                        if (data.data[r].plate_en) {
                                            regname = data.data[r].plate_en;
                                            regid = data.data[r].plate_en;
                                            classes = "carPlates";
                                        }
                                    }
                    ...
                    $('.content-search').delegate('.normal h3','click',function (){

                        if(table == "regions"){
                            var rel = $(this).attr('rel');
                            $('input.search').val($(this).html());
                            $('#firstMap-tab').trigger('click');
                            $('.regionsUl a, path').removeClass('hover');
                            $('#'+rel+'-tab, path.'+rel).addClass('hover');

                            $('.content-search .branch-search').remove();
                            $('.search-in-regions').hide();
                        ...
                        if(table == "branches"){
                            ...
                            $('.content-search .branch-search').remove();
                            $('input.search').val($(this).html());
                            $('#'+rel+'-tab').trigger('click');
                            $('.tab-pane .reg-img img.mark_black').attr('src',urlBase+'mark_black.png');
                            $('.tab-pane .reg-img img[rel='+rel+'_'+$(this).html().replaceAll(' ','_')+']').attr('src',urlBase+'mark_blue.png').closest('.markMap').addClass('animation');
                    ...
                    $('.markMap.animation').removeClass('animation');
                    $('img.mark_black').attr('src',urlBase+'mark_blue.png');
                    $('.nav-link.hover,path.hover').removeClass('hover');
                }
            }
            ...
            $('.regionsUl a').click(function (){
                $('.markMap.animation').removeClass('animation');
                $('img.mark_black').attr('src',urlBase+'mark_blue.png');
                $('.nav-link.hover,path.hover').removeClass('hover');
                $('.nav-link.hover-path,path.hover-path').removeClass('hover-path');

                $('.related-product-block .col-md-9 .carPlatesDataTable').hide();
                $('.related-product-block .col-md-9 .tab-content').show();
                if($(this).attr('id') == "firstMap-tab"){
                    $('#mapIcon').hide();
                }
                else{
                    $('#mapIcon').show();
            ...
            $('path').hover(function (){
                ...
                $('.RegionsNameInMap').show();
                $('.RegionsNameInMap').html(title);
                $('.regionsUl .nav-link').removeClass('hover-path');
                $('#'+rel+'-tab').addClass('hover-path');
            });
            $('path').mouseleave(function (){
                $('.RegionsNameInMap').html(' ').hide();
                $('.nav-link.hover-path').removeClass('hover-path');
            });
            $('path').click(function (){
               var rel = $(this).attr('class');
               var id = rel.replace(' hover','');
            ...
            $('path').mousemove(function (event){
                ...
                $('.RegionsNameInMap').css({'left':(x+10)+'px','top':(y+10)+'px'});
            });

            // #links.show
            $('#links i').click(function (){
                $(this).toggleClass('fa-plus fa-minus').parent().toggleClass('show');
                $('.search-in-regions').hide();
            });
            $('#searchIcon').click(function (){
                $('.search-in-regions').toggle();
                $('#links.show').toggleClass('show');
            });
            $('#mapIcon').hide();
            ...
            $('.content-search ').delegate('.carPlates h3','click',function (){
                var plateEn = $(this).attr('rel');
                $.ajax({
                    data: {'plate_en':plateEn},
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    url: app_url+"/api/map/plates_filter",
                    type: "POST",
                    dataType: 'json',
                    success: function (data) {
                        $('.related-product-block .col-md-9 .carPlatesDataTable table tbody tr').remove();
                        if(table == "carprofiles"){
                            var rel = $(this).attr('rel');

                            $('.related-product-block .col-md-9 .tab-content').hide();
                            for (var q=0;q<data.data.length; q++){
                                var branch = data.data[q].branch.name;
                                var area = data.data[q].BayCode;
                                var checkInDate = data.data[q].checkInDate;
                                var checkOutDate=data.data[q].checkOutDate ;
                                var plateColor= data.data[q].plateColor;
                                var plateCountry= data.data[q].plateCountry;
                                var plateAr = data.data[q].plate_ar;
                                var plateEn = data.data[q].plate_en;
                                //no-car-plate.png
                                var Image;
                                if(data.data[q].screenshot != null){
                                    Image = data.data[q].screenshot;
                                }
                                else {
                                    Image = "/images/no-car-plate.png"
                                }


                                $('.related-product-block .col-md-9 .carPlatesDataTable table tbody').append("<tr><td><img class='no-image' src='"+Image+"' style='padding: 3px' height='40'/> </td><td>"+branch+"</td><td>"+area+"</td><td>"+checkInDate+"</td><td>"+checkOutDate+"</td><td>"+plateColor+"</td><td>"+plateCountry+"</td><td>"+plateAr+"</td><td>"+plateEn+"</td></tr>")
                                // console.log("<tr><td>"+branch+"</td><td>"+checkInDate+"</td><td>"+checkOutDate+"</td><td>"+plateColor+"</td><td>"+plateCountry+"</td><td>"+plateAr+"</td><td>"+plateEn+"</td></tr>")
                            }
                            $('.related-product-block .col-md-9 .carPlatesDataTable').show();
                            $('#mapIcon').show();
                            $('.search-in-regions').hide();
                        }
                    },
            ...
            // carPlatesDataTable image click show
            $('body').delegate('.carPlatesDataTable table img','click',function (){
                var src = $(this).attr('src');
                $('.showImageCarPlates img').attr('src',src);
                $('.showImageCarPlates').show();
            });
            $('.showImageCarPlates .close').click(function(){
                $('.showImageCarPlates').hide();
            })
        });
    </script>
@endpush
