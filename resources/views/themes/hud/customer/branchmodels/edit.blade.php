...
@section('page_title')
    {{__('app.customers.branchmodels.page_title.update')}}
@endsection
@section('meta')
                    ...
                    <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                            <div class="iq-header-title">
                                <h4 class="card-title">{{__('app.customers.branchmodels.page_title.update')}}</h4>
                            </div>
                        </div>
                        <div class="iq-card-body">
                            <div class="new-user-info">
                                <form  method="POST" action="{{ route('branchmodels.update',[$id]) }}">
                                    @csrf
                                        ...
                                        <div class="form-group col-md-12">
                                            ...
                                            <select class="form-control" id="user_model_id" name="user_model_id">
                                                @foreach($models as $model)
                                                    <option  {{$item->user_model_id==$model->id ? 'selected':''}} value="{{$model->id}}">{{$model->model->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group col-md-12">
                                            ...
                                            <select class="form-control" id="branch_id" name="branch_id">
                                                @foreach($branches as $branch)
                                                    <option  {{$item->branch_id==$branch->id ? 'selected':''}} value="{{$branch->id}}">{{$branch->name}}</option>
                                                @endforeach
                                            </select>
