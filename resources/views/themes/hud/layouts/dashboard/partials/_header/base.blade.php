...
{{--                            <i class="fas fa-close" style="display: none"></i>--}}
                    ...
                    <li class="nav-item navDark">
                        @if(session()->has('darkMode'))
                            <a href="{{ route('dark',['off']) }}" class="text-white" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{__('app.Default')}}">
                                <i class="fas fa-sun"></i>
                            </a>
                        @else
                            <a href="{{ route('dark',['on']) }}" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{__('app.Dark')}}">
                    ...
                    <li class="nav-item">
                        @if(str_replace('_', '-', app()->getLocale())=='en')
                            <a class="language-title {{session()->has('darkMode') ? 'whitetext':''}}" href="{{route('select','ar')}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{__('app.Arabic')}}">
                                <i class="fas fa-language"></i>
                                <small>{{ __('app.ar') }}</small>
                            </a>
                        @else
                            <a class="language-title {{session()->has('darkMode') ? 'whitetext':''}}" href="{{route('select','en')}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{__('app.English')}}">
                                <i class="fas fa-language"></i>
            ...
            <ul class="navbar-list">
                <li class="nav-item mr-1">
                    <a href="#" class="search-toggle iq-waves-effect" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{__('app.gym.Notifications')}}">
                        @if(count($oldRequest))
...
{{--                                @endforeach--}}
                ...
                <li class="user-settings">
                    <a href="#" class="search-toggle iq-waves-effect d-flex align-items-center">
                        <img src="{{auth()->user()->avatar ? url('storage/'.auth()->user()->avatar) : url('/images/user/1.jpg') }}" class="img-fluid rounded mr-3" alt="user">
                        <div class="caption">
                    ...
                    <div class="iq-sub-dropdown iq-user-dropdown">
                        <div class="iq-card shadow-none m-0">
                            <div class="iq-card-body p-0 ">
                                ...
                                <a href="{{route('users.changepassword',[auth()->user()->id])}}" class="iq-sub-card iq-bg-primary-hover">
                                    <h6 class=""><i class="ri-profile-line"></i> {{ __('app.change_password') }}</h6>
                                </a>

                                <div class="w-100 p-0">
                                    <a class="iq-sub-card iq-bg-primary-hover border-0"  href="{{ route('logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();" role="button">
