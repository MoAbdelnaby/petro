                                        ...
                                        <table class="table dataTable ui celled table-bordered text-center">
                                            ...
                                            @foreach($users as $user)
                                                <tr class="item{{$user->id}}">
                                                    <td>
                                                        <img src="{{ session()->has('darkMode') ? url('/gym_dark/img'):url('/gym/img') }}/users.svg" alt="product-image" class="img-fluid">
                                                    </td>
                                                    <td>{{$user->name}}</td>
                                                    <td>{{ $user->email }}</td>
                                                    <td>{{ $user->phone??'---' }}</td>
                                                    <td>
                                                        @if(!empty($user->branches))
                                                            @foreach($user->branches as $brn)
                                                                <span class="nav-item li-btn-sm btn-info"><i class="far fa-sitemap"></i> {{$brn->name}}</span>
                                                            @endforeach
                                                        @endif
                                                    </td>
                                                    <td style="width: max-content;display: inline-block">
                                                        @if(auth()->user()->type=="customer" || auth()->user()->type=="subadmin")
                                                            ...
                                                            @endphp
                                                            <a class="btn btn-sm btn-info"  data-toggle="tooltip" data-placement="top" title="" data-original-title="Assign to Branch" onclick="assign_user_to_branch_model_alert({{ $user->id }},{{ $user_brs }},{{ $branches }},{{ $regions }});" style="color: white;">{{__('app.saas.packages.items.Assign_model')}}</a>
                                                            <a class="btn btn-sm btn-primary"  href="{{ route('customerUsers.edit',[$user->id]) }}" style="color: white;">{{__('app.customers.branches.edit')}}</a>
                                                            <a class="btn btn-sm btn-danger" rel="{{ $user->id }}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="delete_alert({{ $user->id }});" >{{__('app.customers.branches.delete')}}</a>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                                                ...
                                                <div class=" iq-card secondary-custom-card h-100" >
                                                    <div class="product-miniature" style="align-content: center;margin: auto;">
                                                        ...
                                                        <div class="product-description">
                                                            <h5><small><span><i class="fas fa-user"></i> {{__('app.users.table.name')}}</span> : {{$user->name}}</small></h5>
                                                            <h5><small><span><i class="fas fa-paper-plane"></i> {{__('app.users.table.email')}}</span> : {{$user->email}}</small></h5>
                                                            <h5><small><span><i class="fas fa-phone-alt"></i> {{__('app.users.table.Phone')}}</span> : {{$user->phone}}</small></h5>

                                                            {{-- <h5><span>{{__('app.saas.packages.items.active_branches')}}</span> : </h5>--}}
                                                            <div class="border-bottom my-1"></div>
                                                            <h5 class="d-block position-relative">
                                                                <small><i class="far fa-sitemap"></i> {{__('app.saas.packages.items.active_branches')}}</small>
                                                                <span class="float-right showbranchesAll"><i class="fas fa-info"></i></span>
                                                                {{--                                                                                @if(!empty($item->itembranches) and count($item->itembranches) > 5 )--}}
                                                                <div class="branchesAll">

                                                                    <div class="content-branches">
                                                                        @foreach($user->branches as $brn)
                                                                            <h6><i class="far fa-sitemap"></i> {{$brn->name}}</h6>
                                                                        @endforeach
                                                                    </div>
                                                            ...
                                                            <div class="position-relative">
                                                                <ul class="ratting-item scroll-vertical-custom d-flex p-0 m-0">
                                                                    <div class="scroll-vertical-custom-div">
                                                                        @if(!empty($user->branches))
                                                                            @foreach($user->branches as $brn)
                                                                                <li class="nav-item li-btn-sm btn-info"><i class="far fa-sitemap"></i> {{$brn->name}}</li>
                                                                            @endforeach
                                                                        @endif
                                                                      </div>
                                                                </ul>
                                                    ...
                                                    <div class="col-12">
                                                        <div class="text-center px-0 pb-2">
                                                            @if(auth()->user()->type=="customer" || auth()->user()->type=="subadmin")
                                                                ...
                                                                @endphp
                                                                <a class="btn btn-info"  data-toggle="tooltip" data-placement="top" title="" data-original-title="{{ __('app.Assign_to_Branch') }}" onclick="assign_user_to_branch_model_alert({{ $user->id }},{{$user_brs}},{{$branches}},{{$regions}});" style="color: white;">{{__('app.saas.packages.items.Assign_model')}}</a>
                                                                <a class="btn btn-primary"  href="{{ route('customerUsers.edit',[$user->id]) }}" style="color: white;">{{__('app.customers.branches.edit')}}</a>
                                                                <a class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="delete_alert({{ $user->id }});" >{{__('app.customers.branches.delete')}}</a>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
        ...
        <div class="modal-dialog modal-lg" style="margin-top: 100px">
            <div class="modal-content">
                <div class="modal-header row px-0 mx-0">
                    <h5 class="modal-title col"><i class="fas fa-sitemap"></i> {{__('app.customers.branchmodels.page_title.create')}}</h5>
                    <div class="col">
                        <input type="search" id="searchBranch" class="float-right form-control" placeholder="{{ __('app.search_in_branches_or_regions') }}">
                    </div>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">{{__('app.customers.branches.close')}}</button>
                    <button type="button" class="btn btn-primary" onclick="assign_option();">{{__('app.customers.branchmodels.save')}}</button>
                </div>
            </div>
        </div>
